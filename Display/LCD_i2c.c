...
void LCD_Init(LCD_I2C_t *lcd_i2c, uint8_t addr, uint8_t cols, uint8_t rows)
{
	...
	LCD_command(lcd_i2c, LCD_5x8DOTS|LCD_2LINE);
 ...
 * @par command  - command
 * @retval 1 - success, 0 - error device not responding
 */
uint8_t LCD_command(LCD_I2C_t *lcd_i2c, uint8_t command)
{
 ...
 * @return 1 - success, 0 - error device not responding
 */
uint8_t LCD_send(LCD_I2C_t *lcd_i2c, uint8_t data, uint8_t flags)
{
	...
	// send again, this time EN is zero
